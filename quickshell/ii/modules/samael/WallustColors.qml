...
Singleton {
    ...
    readonly property color text: { const _b = paletteGeneration; return _color("text", "#FFFFFF") }
    readonly property color moduleText: Qt.color("#e5d9f5")
    ...
    Process {
        ...
        function readNow() {
            exec(["bash", "-c",
                "W='/home/samael/.config/waybar/wallust/colors-waybar.css'; "
                + "L='/home/samael/.config/waybar/style/catppuccin-themes/latte.css'; "
                + "printf '%s\\n%s' \"$(cat \"$W\" 2>/dev/null)\" \"$(cat \"$L\" 2>/dev/null)\""])
        }

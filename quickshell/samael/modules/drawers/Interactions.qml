...
CustomMouseArea {
    ...
    onContainsMouseChanged: {
        ...
        onPositionChanged: event => {
            ...
            // Samael: no Caelestia hover chrome (dashboard / sidebar / utilities / osd / notifs)
            if (!panels.caelestiaChromeEnabled) {
                if (!Config.launcher.showOnHover && !Config.launcher.enabled)
                    return;
                if (Config.launcher.enabled && Config.launcher.showOnHover && !screenState.launcher && inBottomPanel(panels.launcher, x, y))

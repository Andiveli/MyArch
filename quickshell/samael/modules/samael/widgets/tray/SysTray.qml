...
Item {
    ...
    // Samael / Waybar-like: all tray icons on the bar (no expand_more)
    property bool showAllItemsInline: false
    property int trayIconSpacing: 15
    property var activeMenu: null

    property list<var> pinnedItems: TrayService.pinnedItems
    property list<var> unpinnedItems: TrayService.unpinnedItems
    property list<var> inlineTrayItems: {
        if (!root.showAllItemsInline)
            return root.pinnedItems
        const smart = Config.options.tray.filterPassive
        return SystemTray.items.values.filter(i => !smart || i.status !== Status.Passive)
    }
    ...
    GridLayout {
        ...
        anchors.fill: parent
        rowSpacing: 8
        columnSpacing: root.trayIconSpacing

        RippleButton {
            id: trayOverflowButton
            visible: root.showOverflowMenu && !root.showAllItemsInline && root.unpinnedItems.length > 0

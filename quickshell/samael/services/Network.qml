...
Singleton {
    ...
    function changePassword(network: WifiAccessPoint, password: string, username = ""): void {
        ...
        if (username.length > 0) {
            applyEnterpriseWifi(network, username, password)
            return
        }
    ...
    /** WPA2-Enterprise (PEAP) — vim-friendly panel; exotic EAP still needs nm-connection-editor */
    function applyEnterpriseWifi(network: WifiAccessPoint, username: string, password: string): void {
        network.askingPassword = false;
        enterpriseWifiProc.exec({
            "environment": {
                "PASSWORD": password,
                "USER": username,
                "SSID": network.ssid
            ...
            "command": ["bash", "-c",
                'nmcli connection modify "$SSID" 802-1x.identity "$USER" 802-1x.password "$PASSWORD" wifi-sec.key-mgmt wpa-eap 802-1x.eap peap 2>/dev/null; '
                + 'nmcli connection up "$SSID" 2>/dev/null || nmcli dev wifi connect "$SSID"']
        })
    }

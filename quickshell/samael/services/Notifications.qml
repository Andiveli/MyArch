...
Singleton {
    ...
    function notifToString(notif) {
        ...
        readonly property bool samaelPanel: (Config?.options?.panelFamily ?? "") === "samael"

        function syncKey(notification) {
            const h = notification?.hints
            if (!h)
                return ""
            const sk = h["x-canonical-private-synchronous"]
            if (sk !== undefined && sk !== null && String(sk).length > 0)
                return String(sk)
            return ""
        }

        function volumeBodyFromHints(notification, summary, body) {
            const h = notification?.hints
            const v = h?.value
            if (v === undefined || v === null || isNaN(Number(v)))
                return (body || "").trim()
            const s = (summary || "").toLowerCase()
            if (!s.includes("volume") && !s.includes("mic") && s !== "mute")
                return (body || "").trim()
            const pct = Number(v)
            return pct === 0 ? "Muted" : (Math.round(pct) + "%")
        }

        function findBySyncKey(key) {
            if (!key.length)
                return -1
            for (let i = root.list.length - 1; i >= 0; i--) {
                const w = root.list[i]
                if (syncKey(w.notification) === key)
                    return i
            }
            return -1
        }

        function refreshPopupTimer(notifObject, notification) {
        ...
        component NotifTimer: Timer {
            ...
            onTriggered: () => {
                ...
                if (notifObject?.isTransient && !root.samaelPanel)
                    root.discardNotification(notificationId);
                else
	...
	NotificationServer {
        ...
        onNotification: (notification) => {
            ...
            const mergedBody = root.volumeBodyFromHints(notification, notification.summary, notification.body)
            const sk = root.syncKey(notification)
            const replaceIdx = root.samaelPanel ? root.findBySyncKey(sk) : -1

            if (replaceIdx >= 0) {
                const existing = root.list[replaceIdx]
                if (existing.timer)
                    existing.timer.destroy()
                existing.notification = notification
                existing.summary = notification.summary
                existing.body = mergedBody.length ? mergedBody : notification.body
                existing.time = Date.now()
                if (!root.popupInhibited) {
                    existing.popup = true
                    root.refreshPopupTimer(existing, notification)
                }
                root.notify(existing)
            ...
            if (mergedBody.length)
                newNotifObject.body = mergedBody
            ...
            if (!root.popupInhibited) {
                ...
                if (notification.expireTimeout != 0)
                    root.refreshPopupTimer(newNotifObject, notification)
                root.unread++;
            }

...
Singleton {
    ...
    readonly property url defaultFolder: Config.options.panelFamily === "samael"
        ? Qt.resolvedUrl(FileUtils.trimFileProtocol(Directories.pictures) + "/wallpapers")
        : Qt.resolvedUrl(`${Directories.pictures}/Wallpapers`)
    ...
    function openFallbackPicker(darkMode = Appearance.m3colors.darkmode) {
        if (Config.options.panelFamily === "samael") {
    ...
    function apply(path, darkMode = Appearance.m3colors.darkmode) {
        ...
        if (Config.options.panelFamily === "samael") {
            const script = `${FileUtils.trimFileProtocol(Directories.scriptPath)}/wallpaper/samael-wallpaper.sh`;
            Quickshell.execDetached(["bash", script, FileUtils.trimFileProtocol(path)]);
        } else {

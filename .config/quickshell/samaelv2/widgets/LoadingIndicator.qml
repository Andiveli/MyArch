...
/**
 * Caelestia-style loading indicator — MaterialShape morph + rotate animation.
 * Adapted from caelestia-shell/components/controls/LoadingIndicator.qml
 * (replaced qs.components/qs.services deps with local equivalents).
 */
MaterialShape {
    id: root
    ...
    /** Media accent (WallustColors.sky matches media panel highlight). */
    property alias indicatorColor: root.color
